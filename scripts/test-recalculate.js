...
async function main() {
    ...
    const loginRes = await fetch(`${baseUrl}/api/mobile/auth/login`, {
        ...
        body: JSON.stringify({ email: 'admin@nexorfire.com', password: 'admin' })
    });
    ...
    console.log('Current Stop Order:', route.stops.map(s => s.inspection.building.name).join(' -> '));
    ...
    console.log('New Stop Order:', updatedRoute.stops.map(s => s.inspection.building.name).join(' -> '));
}
